...
// Login activity log
const LOGIN_LOG_FILE = path.join(DATA_DIR, 'logins.json');
function loadLoginLog() { try { if (fs.existsSync(LOGIN_LOG_FILE)) return JSON.parse(fs.readFileSync(LOGIN_LOG_FILE, 'utf8')); } catch {} return []; }
function logLogin(user, ip) {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  const log = loadLoginLog();
  log.push({ user, ip, time: new Date().toISOString() });
  // Keep last 200 entries
  const trimmed = log.slice(-200);
  try { fs.writeFileSync(LOGIN_LOG_FILE, JSON.stringify(trimmed, null, 2)); } catch {}
}
function getActiveSessions() {
  const now = Date.now();
  const active = {};
  for (const [token, s] of Object.entries(sessions)) {
    if (now < s.expires) active[s.user] = (active[s.user] || 0) + 1;
  }
  return active;
}

// Per-county data
const VALID_COUNTIES = ['iron', 'washington', 'kane', 'duchesne'];
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/login' && req.method === 'POST') {
    ...
    if (USERS[body.user] && USERS[body.user] === hashPassword(body.pass || '')) {
      logLogin(body.user, ip);
      return send(res, 200, { token: createSession(body.user) });
    }
  ...
  // Login activity log
  if (url.pathname === '/api/activity') return send(res, 200, { logins: loadLoginLog().slice(-50).reverse(), active: getActiveSessions() });
